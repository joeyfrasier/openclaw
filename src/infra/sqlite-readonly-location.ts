...
import {
  reclaimStalePrivateSqliteSnapshotsSync,
  sqliteSnapshotStagingPrefix,
} from "./sqlite-snapshot-reclamation.js";
...
async function createSqliteSnapshotStagingDirectory(ownerPid: number): Promise<string> {
  ...
  try {
    ...
    return await createPrivateSqliteTempDirectory(
      stagingRoot,
      sqliteSnapshotStagingPrefix(ownerPid),
    );
  } catch (error) {
