...
describe("prepareCliRunContext", () => {
  ...
  it.each([true, false])(
    "removes only the message tool with explicit availability %s",
    async (explicitAvailability) => {
      const prepareExecution = vi.fn(async () => ({ toolAvailabilityEnforced: true as const }));
      const mintMcpLoopbackClientGrant = vi.fn(createTestMcpLoopbackClientGrant);
      setRawCliBackendForPrepareTest({
        id: "claude-cli",
      ...
      const expectedAvailability = explicitAvailability
        ? { native: ["Read"], openClaw: ["read"] }
        : undefined;
      expect(context.params.cliToolAvailability).toEqual(expectedAvailability);
      expect(prepareExecution).toHaveBeenCalledWith(
        expect.objectContaining({
          toolAvailability: expectedAvailability,
        }),
      );
